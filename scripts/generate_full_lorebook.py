...
SOURCE_RULES_LOREBOOK_PATH = ROOT / "sillytavern" / "import" / "崇祯历史模拟器_资料规则Lorebook.json"
...
def main() -> None:
    ...
    if (
        QUICK_LOREBOOK_PATH.exists()
        or SOURCE_RULES_LOREBOOK_PATH.exists()
        or SCENARIO_LOREBOOK_PATH.exists()
        or CHARACTER_LOREBOOK_PATH.exists()
    ):
        complete_entries: OrderedDict[str, OrderedDict] = OrderedDict(entries)
        next_uid = max(int(key) for key in complete_entries) + 1
        for lorebook_path in (
            QUICK_LOREBOOK_PATH,
            SOURCE_RULES_LOREBOOK_PATH,
            SCENARIO_LOREBOOK_PATH,
            CHARACTER_LOREBOOK_PATH,
        ):
